...
frog_plot <- ggplot(summary_frogs, aes(x= year, y= count))+
  ...
  labs(title = str_wrap("Total counts across all lake locations per year for each lifestage", 100),
       x= "Year",
       y= "Count")+
  theme_minimal()+
...
plot_lakes <- lake_graph_data_ordered %>% 
  ...
  geom_col_pattern(aes(pattern_density = amphibian_life_stage, pattern_fill = as.factor(lake_id), pattern_color = as.factor(lake_id)),
                   ...
                   fill    = 'white',
                   color = 'black')+
  labs(title= str_wrap("Total counts of adults and subadults in lakes with highest frog abundance", 100),
       x= "Lake",
       y= "Count",
       pattern_density= "Life Stage",
       pattern_color= 'Lake ID Number')+
  theme_minimal()+
  theme(axis.text.x = element_blank(), plot.title = element_text(size=13), legend.box = "horizontal")+
